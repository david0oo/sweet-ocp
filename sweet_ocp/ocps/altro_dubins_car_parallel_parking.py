...
@dataclass
class DubinsCarParallelParkingParameters:
    """
    Parameters for the Dubins car parallel parking problem.
    """
    N: int = 101
    nx: int = 3
    nu: int = 2
    ...
    terminal_state = np.array([0.0, 1.5, 0.0])
    u_min = np.array([-2.0, -2.0])
    u_max = np.array([2.0, 2.0])
    x_min = np.array([-0.25, -0.001, -ACADOS_INFTY])
    x_max = np.array([0.25, 1.5 + 0.001, ACADOS_INFTY])
    u_init = np.array([0.1, 0.1])


def create_problem(opts: AcadosOcpOptions, mode: str = 'FIXED_TIME'):
    """
    Create the optimal control problem (OCP) for the Dubins car parallel parking problem.
    ...
    """
    params = DubinsCarParallelParkingParameters()
    ...
    # Path constraints on controls
    ocp.constraints.lbx = params.x_min
    ocp.constraints.ubx = params.x_max
...
def plot_trajectory(sol_X, sol_U) -> None:
    """
    Plot the trajectory of the Dubins car.

    Args:
        params (DubinsCarParallelParkingParameters): Problem parameters.
        sol_x_y (np.ndarray): Solution trajectory in the x-y plane.
    """
    # params = DubinsCarParallelParkingParameters()
    sol_x_y = sol_X[:,:2]
    _, ax = plt.subplots()
